...
class EpoxyVisibilityTracker {
    ...
    private val visibilityIdToItems = ArrayList<EpoxyVisibilityItem>()
    ...
    private var lastAdapterSeen: Adapter<*>? = null

    /** All nested visibility trackers  */
    private val nestedTrackers = HashMap<RecyclerView, EpoxyVisibilityTracker>()
    ...
    private fun processChangeEvent(debug: String) {
    ...
    private fun processVisibilityEvents(
        ...
        if (DEBUG_LOG) {
            Log.d(
                TAG,
                "$eventOriginForDebug.processVisibilityEvents ${System.identityHashCode(epoxyHolder)}, $detachEvent, ${epoxyHolder.adapterPosition}"
        ...
        var vi: EpoxyVisibilityItem? = visibilityIdToItemMap.get(id)
        if (vi == null) {
        ...
        } else if (epoxyHolder.adapterPosition != RecyclerView.NO_POSITION && vi.adapterPosition != epoxyHolder.adapterPosition) {
     ...
     * implementations
     */
    private inner class Listener : OnScrollListener(), OnLayoutChangeListener,
    ...
    companion object {
        ...
        private val TAG = "EpoxyVisibilityTracker"
